	...
	aMenu={
		...
		init: function(elem,settings) {
			...
			$(this.menu).find('li').each(function(){
				...
				if (childs.length > 0) {
					...
					$(this).children('a').addClass('parent-link');
				}
		...
		fix: function(elem) {
			...
			if (p.length>1) {
				...
				if (aMenu.params.direction == 'rtl') {					
					$(elem).css({
								'right': $(elem).parent('li').width() + 2,
								'top': "10px"
								});					
				} else if (aMenu.params.direction == 'ltr'){
					
					$(elem).css({
								'left': $(elem).parent('li').width() + 2,
								'top': "10px"
								});						
				}
